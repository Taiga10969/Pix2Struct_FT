...
class Pix2StructVisionAttention(nn.Module):
    ...
    def forward(
        ...
        encoder_attn_info=None, # encoder_attn_info
    ):
        """
        ...
        #print("Pix2StructVisionAttention [info] encoder_attn_info : ", encoder_attn_info)
        ...
        # 現状はソフトマックス取った後のAttention Weightに対して値を加算する
        if encoder_attn_info is not None: # torch.Size([1, 12, 2048])
            #print("Pix2StructVisionAttention [info] attn_weights[0][0][0][:10] : ", attn_weights[0][0][0][:10])
            encoder_attn_info = encoder_attn_info.unsqueeze(2) # torch.Size([1, 12, 1, 2048])
            encoder_attn_info = encoder_attn_info.expand(-1, -1, seq_length, -1) ## torch.Size([1, 12, 2048, 2048])
            attn_weights += encoder_attn_info
            #print("Pix2StructVisionAttention [info] attn_weights[0][0][0][:10] : ", attn_weights[0][0][0][:10])
        
        #print("Pix2StructVisionAttention [info] attn_weight.shape : ", attn_weights.shape) # torch.Size([1, 12, 2048, 2048])
...
class Pix2StructVisionLayer(nn.Module):
    ...
    def forward(
        ...
        #print("Pix2StructVisionLayer [info] encoder_attn_info : ", encoder_attn_info)
        ...
        self_attention_outputs = self.attention(
            ...
            encoder_attn_info = encoder_attn_info # encoder_attn_info
        )
...
class Pix2StructVisionEncoder(nn.Module):
    ...
    def forward(
        ...
        #print("Pix2StructVisionEncoder [info] encoder_attn_info : ", encoder_attn_info)
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        return module(*inputs, output_attentions=output_attentions, encoder_attn_info=encoder_attn_info)
                ...
                layer_outputs = layer_module(hidden_states, attention_mask, layer_head_mask, output_attentions, encoder_attn_info)

            hidden_states = layer_outputs[0]

            if output_attentions:
                all_self_attentions = all_self_attentions + (layer_outputs[2],) # get_attention_modi
...
class Pix2StructForConditionalGeneration(Pix2StructPreTrainedModel):
    ...
    def __init__(self, config: Pix2StructConfig):
        ...
        #print("Pix2StructForConditionalGeneration [info] config.vision_config", config.vision_config)
        #rint("Pix2StructForConditionalGeneration [info] config.text_config", config.text_config)
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        encoder_attn_info = None, # encoder_attn_info  torch.Size([1, 12, 2048])のデータを入力 (batch, head, w)
        ...
        #print("Pix2StructForConditionalGeneration [info] call forward()")
        ...
        #print("Pix2StructForConditionalGeneration [info] hidden_states.shape : ", hidden_states.shape) # torch.Size([1, 2048, 768]) 全てのパッチ情報を伝播していることを確認
