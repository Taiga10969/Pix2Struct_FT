...
class Pix2StructImageProcessor(BaseImageProcessor):
    ...
    def extract_flattened_patches(
        ...
        **kwargs,
        ...
        #print('My log >> call extract_flattened_patches()')
        """
        ...
        #print('result.shape : ', result.shape) # torch.Size([1978, 770]) # row * columns, depth+2

        # [max_patches, 2 + patch_height * patch_width * image_channels]
        #print("before padding : ", result)
        result = torch.nn.functional.pad(result, [0, 0, 0, max_patches - (rows * columns)]).float() # max_patchesに足りない分をpadding
        #print("after padding : ", result)

        result = to_numpy_array(result)

        return result, {"rows": rows, "columns": columns, "depth": depth, "patches": patches}
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        #print('input_data_format : ', input_data_format)
        # convert to torch tensor and permute
        results_and_rows_cols = [self.extract_flattened_patches(image=image, max_patches=max_patches, patch_size=patch_size, input_data_format=input_data_format) for image in images]
        images = [result for result, _ in results_and_rows_cols]
        extract_flattened_patches_info = [rows_cols for _, rows_cols in results_and_rows_cols]
        ...
        return encoded_outputs, {"normalize_images": normalize_images, "extract_flattened_patches_info": extract_flattened_patches_info}